...
Band `290-visual`. Chat kernel is the `update(t)` posted in-session (uniforms → theta → geometry switch → lerp 0.05).
Session kernel still allocates `new THREE.Vector3` inside lerp; live path uses a reused `_kernelTarget` / `_target`.
Stage 290 reconfirms the session paste (`beec41f1`) from 2026-09-25 16:07 CDT. No new case labels.
...
| 72–289 | Live chat + remembral stamps. Session pin `beec41f1` held. |
| 290 | Live chat 2026-09-25 16:07 CDT. Same four-geometry paste. Docs + STAGE bump. |
...
| 291 | Cryptic-Heartbeat / continuity-mesh-speedway | Next waterfall hop. |

## Drive from LLM

```js
window.dispatchEvent(new CustomEvent('gaia:targetState', {
  detail: { geometry: 'blend', gravityPull: 1.4, toroidalWeave: 1.2, lerp: 0.05, blend: 0.6, stage: 290, sourceHash: '7cd81012' }
}));
```
